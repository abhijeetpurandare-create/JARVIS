...
const NavItem = ({ icon, label, active }: NavItemProps) => (
  <button
    className={`flex items-center justify-center w-[44px] h-[36px] rounded-tds-md cursor-pointer transition-colors ${
      active ? 'bg-tds-surface-bg-coal-weak' : 'hover:bg-tds-surface-bg-coal-weakest'
    }`}
    aria-label={label}
    title={label}
  >
    <span className="text-tds-icon-body-primary">{icon}</span>
  </button>
);

const SideNavigation = () => {
  return (
    <nav className="flex flex-col items-center h-full w-[60px] bg-tds-surface-bg-primary-default border-r border-tds-border-neutral-primary pt-tds-8 pb-tds-16 px-tds-8">
      {/* Top nav items */}
